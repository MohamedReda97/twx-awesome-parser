...
// Business Object Builder functionality
let selectedTWXFileForBuilder = null;
let businessObjectBuilder = null;

/**
 * Initialize Business Object Builder
 */
function initializeBusinessObjectBuilder() {
    // This would be called when the backend is available
    console.log('Business Object Builder initialized');
}

/**
 * Insert example JSON into the textarea
 */
...
function validateJSON() {
    ...
    if (!jsonText) {
        hideJSONValidation();
        jsonInput.className = 'json-textarea';
        return false;
    }
    ...
    try {
        ...
        // Validate structure
        const validation = validateBusinessObjectStructure(parsed);
        
        if (validation.isValid) {
            showJSONValidation(true, `✅ Valid JSON structure with ${validation.objectCount} business objects`);
            jsonInput.className = 'json-textarea valid';
            return true;
        } else {
            showJSONValidation(false, `❌ Invalid structure: ${validation.errors.join(', ')}`);
            jsonInput.className = 'json-textarea invalid';
            return false;
        }
        
    } catch (error) {
        showJSONValidation(false, `❌ Invalid JSON: ${error.message}`);
        jsonInput.className = 'json-textarea invalid';
        return false;
    }
}
 ...
 */
function validateBusinessObjectStructure(parsed) {
    const result = {
        isValid: true,
        errors: [],
        objectCount: 0
    ...
    const objectNames = Object.keys(parsed);
    if (objectNames.length === 0) {
        result.isValid = false;
        result.errors.push('At least one business object must be defined');
        return result;
    }
    
    result.objectCount = objectNames.length;
    
    for (const objectName of objectNames) {
        const objectDef = parsed[objectName];
        
        if (typeof objectDef !== 'object' || objectDef === null) {
            result.isValid = false;
            result.errors.push(`Object '${objectName}' must be an object`);
            continue;
        }
        
        const properties = Object.keys(objectDef);
        if (properties.length === 0) {
            result.errors.push(`Object '${objectName}' has no properties (warning)`);
        }
        
        for (const propName of properties) {
            const propType = objectDef[propName];
            if (typeof propType !== 'string') {
                result.isValid = false;
                result.errors.push(`Property '${objectName}.${propName}' type must be a string`);
            }
        }
    }
...
/**
 * Show JSON validation message
 */
function showJSONValidation(isValid, message) {
    const validationDiv = document.getElementById('json-validation');
    validationDiv.style.display = 'block';
    validationDiv.className = `json-validation ${isValid ? 'valid' : 'invalid'}`;
    validationDiv.textContent = message;
}

/**
 * Hide JSON validation message
 */
...
/**
 * Select TWX file for builder
 */
function selectTWXFileForBuilder() {
    const fileInput = document.getElementById('twx-file-input-builder');
    fileInput.click();
    ...
    fileInput.onchange = function(event) {
        ...
        if (file) {
            if (file.name.toLowerCase().endsWith('.twx')) {
                selectedTWXFileForBuilder = file;
                document.getElementById('selected-twx-file-builder').textContent = `Selected: ${file.name}`;
                document.getElementById('clear-twx-builder').style.display = 'inline-block';
                updateGenerateButtonState();
                updateStatus(`Builder: Selected TWX file: ${file.name}`);
            } else {
                alert('Please select a valid TWX file (.twx extension)');
                clearTWXFileSelection();
            }
        }
    };
}
...
/**
 * Clear TWX file selection for builder
 */
function clearTWXFileSelection() {
    selectedTWXFileForBuilder = null;
    document.getElementById('selected-twx-file-builder').textContent = '';
    document.getElementById('clear-twx-builder').style.display = 'none';
    document.getElementById('twx-file-input-builder').value = '';
    updateGenerateButtonState();
    updateStatus('Builder: TWX file selection cleared');
}
...
function updateGenerateButtonState() {
    ...
    const hasValidJSON = jsonInput && validateJSON();
    const hasTWXFile = selectedTWXFileForBuilder !== null;
    
    generateBtn.disabled = !(hasValidJSON && hasTWXFile);
}

/**
 * Preview business objects (without adding to TWX)
 */
async function previewBusinessObjects() {
    const jsonInput = document.getElementById('json-input').value.trim();
    
    if (!jsonInput) {
        alert('Please enter JSON object definitions');
        return;
    }
    
    if (!validateJSON()) {
        alert('Please fix JSON validation errors first');
        return;
    }
    ...
    try {
        ...
        const previewData = await generateBusinessObjectsAPI(jsonInput, false);
        
        displayGenerationResults(previewData, true);
        updateStatus('Preview generated successfully');
        
    } catch (error) {
        console.error('Preview generation failed:', error);
        alert(`Preview failed: ${error.message}`);
        updateStatus('Preview generation failed');
    }
}
 ...
 */
async function generateBusinessObjects() {
    const jsonInput = document.getElementById('json-input').value.trim();
    ...
    if (!selectedTWXFileForBuilder) {
        alert('Please select a TWX file');
        return;
    }
    
    try {
        updateStatus('Generating business objects...');
        ...
        // Call real API for generation
        const generationData = await generateBusinessObjectsAPI(jsonInput, true, options);
        
        displayGenerationResults(generationData, false);
        
        if (generationData.downloaded) {
            updateStatus('Business objects generated and TWX file downloaded successfully');
        } else {
            updateStatus('Business objects generated successfully');
        }
        
        // Optionally refresh the main object browser if objects were added
        if (generationData.success && generationData.addedObjects > 0) {
            setTimeout(() => {
                if (confirm('Business objects were added to the TWX file. Would you like to reload the object browser to see them?')) {
                    // This would trigger a reload of the main TWX parser
                    location.reload();
                }
            }, 2000);
        }
        ...
        updateStatus('Business object generation failed');
    }
}

/**
 * Call the real Business Object Builder API
 */
async function callBusinessObjectAPI(endpoint, data = null, method = 'GET') {
    try {
        const options = {
            method: method,
            headers: {
        ...
        const response = await fetch(`/api/business-objects/${endpoint}`, options)
...
/**
 * Generate business objects using real API
 */
async function generateBusinessObjectsAPI(jsonInput, addToTWX = false, options = {}) {
    if (!addToTWX) {
        // Preview mode - call preview endpoint
        const result = await callBusinessObjectAPI('preview', { jsonInput }, 'POST')
        
        if (result.success && result.preview) {
            // Transform preview data to match expected format
            return {
                success: true,
                inputObjects: result.count,
                generatedObjects: result.preview.filter(p => !p.error).length,
                addedObjects: 0,
                failedObjects: result.preview.filter(p => p.error).length,
                generatedBusinessObjects: result.preview.map(p => ({
                    name: p.name,
                    id: p.id || 'preview-id',
                    properties: p.properties ? p.properties.length : 0,
                    xmlSize: 0
                })),
                errors: result.preview.filter(p => p.error).map(p => p.error),
                warnings: []
            }
        }
        ...
        // Generation mode - call generate endpoint with file upload
        if (!selectedTWXFileForBuilder) {
            return {
                success: false,
                error: 'No TWX file selected'
            }
        }
        
        try {
            const formData = new FormData()
            formData.append('twxFile', selectedTWXFileForBuilder)
            formData.append('jsonInput', jsonInput)
            formData.append('options', JSON.stringify(options))
            
            const response = await fetch('/api/business-objects/generate', {
                method: 'POST',
                body: formData
            })
            ...
            if (response.ok) {
                ...
                if (contentType && contentType.includes('application/octet-stream')) {
                    ...
                    a.href = url
                    a.download = selectedTWXFileForBuilder.name
                    document.body.appendChild(a)
                    a.click()
                    document.body.removeChild(a)
                    window.URL.revokeObjectURL(url)
                    
                    // Return success result for UI
                    const parsed = JSON.parse(jsonInput)
                    const objectNames = Object.keys(parsed)
                    
                    return {
                        success: true,
                        inputObjects: objectNames.length,
                        generatedObjects: objectNames.length,
                        addedObjects: objectNames.length,
                        failedObjects: 0,
                        generatedBusinessObjects: objectNames.map(name => ({
                            name: name,
                            id: 'generated-id',
                            properties: Object.keys(parsed[name]).length,
                            xmlSize: 0
                        })),
                        errors: [],
                        warnings: [],
                        downloaded: true,
                        message: 'TWX file generated and downloaded successfully'
                    }
                } else {
                    return await response.json()
                }
...
/**
 * Generate mock UUID for simulation
 */
 ...
 */
function displayGenerationResults(results, isPreview = false) {
    ...
    const summaryHTML = `
        <div class="result-summary">
            <div class="result-stat">
                <span class="result-stat-value">${results.inputObjects}</span>
                <span class="result-stat-label">Input Objects</span>
            </div>
            <div class="result-stat">
                <span class="result-stat-value">${results.generatedObjects}</span>
                <span class="result-stat-label">Generated</span>
            </div>
            ${!isPreview ? `
                <div class="result-stat">
                    <span class="result-stat-value">${results.addedObjects}</span>
                    <span class="result-stat-label">Added to TWX</span>
                </div>
            ` : ''}
            <div class="result-stat">
                <span class="result-stat-value">${results.failedObjects}</span>
                <span class="result-stat-label">Failed</span>
            </div>
        </div>
    `;
    
    const objectsHTML = results.generatedBusinessObjects ? `
        <div class="result-objects">
            <h5>${isPreview ? '👁️ Preview:' : '✅ Generated:'} Business Objects</h5>
            ${results.generatedBusinessObjects.map(obj => `
                <div class="result-object">
                    <div class="result-object-header">
                        <span class="result-object-name">${obj.name}</span>
                        <span class="result-object-id">${obj.id}</span>
                    </div>
                    <div class="result-object-details">
                        Properties: ${obj.properties} | XML Size: ${obj.xmlSize.toLocaleString()} characters
                    </div>
                </div>
            `).join('')}
        </div>
    ...
    const warningsHTML = results.warnings && results.warnings.length > 0 ? `
    ...
    resultsContent.innerHTML = summaryHTML + objectsHTML + errorsHTML + warningsHTML;
    resultsDiv.style.display = 'block';
...
/**
 * Toggle type reference visibility
 */
function toggleTypeReference() {
